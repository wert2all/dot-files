...
get_audio_sinks() {
    pactl list sinks | grep -E "(Sink #|Description:|Name:)" |
        ...
        /Description:/ { 
            ...
            print sink_id "'"$DELIMITER"'" name "'"$DELIMITER"'" desc
        }
    '
}
...
main() {
    ...
    sink_list=""
    while IFS="$DELIMITER" read -r sink_id sink_name description; do
        # Mark current default with an indicator
        indicator=""
        if [ "$sink_name" = "$current_sink" ]; then
            indicator=" [CURRENT]"
        fi
        sink_list+="$sink_id$DELIMITER$sink_name$DELIMITER$description$indicator"$'\n'
    done <<<"$sinks"
    ...
    selected_sink=$(echo "$selected" | cut -d$DELIMITER -f2)
    selected_desc=$(echo "$selected" | cut -d$DELIMITER -f3 | sed 's/ \[CURRENT\]$//')
